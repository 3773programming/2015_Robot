...
public class Robot extends IterativeRobot {
	...
	AxisCamera AC2 = new AxisCamera("10.37.73.11");
	public static AxisCamera.Resolution k640x360;
	boolean B1 = DJ.getRawButton(8);
	/**
...
public void autonomousPeriodic() {
	...
	ti = t.get();
	while (ti < 3) { //Make a Loop to Run the Autonomous Code
	Drive.drive(-.5, -.025); //Drive Forward at Half Speed Until the Timer Reaches Two
	ti = t.get();
	}
	Drive.stopMotor();
	t.stop();
...
public void teleopPeriodic() {
	...
	B1 = DJ.getRawButton(8);
	while(B1 == true){
		Drive.arcadeDrive(DY*.5, -DX*.5);
		A1.set(AY);
		B1 = DJ.getRawButton(8);
		AY = AJ.getY();
		DY = DJ.getY();
